...
// V2.1B – Asignación de personal a recursos con filtros territoriales por rol
//
// admin:                   Región → Cantonal → Subdelegación
// jefatura / UO cantonal:  Solo subdelegación (todas las de su cantonal)
// jefatura_dist / UO_dist: Solo su delegación (badge fijo)
// supervisor:              Sin filtros → ve recursos de su delegación directamente
// agente:                  Sin acceso
...
const ROLES_CON_FILTRO_SUBDELEGACION = [
  "jefatura","unidad_operativa",
...
function GestionRecurso() {
  ...
  const tieneFiltroCantonal = esAdmin || esCantonal;
  const tieneFiltroBadge    = esDistrital;
  ...
  // ── Datos al cambiar subdelegación (admin y cantonal) ─
  useEffect(() => {
  ...
  const delegActiva = filtroSubdeleg ||
    (esDistrital || esSupervisor ? userData?.delegation_id : "");

  const recargar = useCallback(async () => {
    if (delegActiva) await cargarDatosDelegacion(delegActiva);
  }, [delegActiva]);
  ...
  async function liberarRecurso() {
    if (!recursoSeleccionado) return;
    if (!confirm(
      `¿Liberar ${recursoSeleccionado.nombre_recurso}?\nSe removerán todos los funcionarios asignados y el recurso quedará disponible.`
    )) return;
    setLoading(true); setError("");
    try {
