      ...
      {loading ? (
        ...
        <div style={tableWrap}>
          <table style={tableStyle}>
            <thead>
              <tr>{["Orden","Nombre","Siglas","Descripción","Estado",""].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {rangos.map(r => (
                <tr key={r.id} style={r.estado === "inactivo" ? { opacity: 0.5 } : {}}>
                  ...
                  <td style={tdStyle}><span style={r.estado === "activo" ? badgeActiveStyle : badgeInactiveStyle}>{r.estado}</span></td>
                  <td style={tdStyle}>{r.estado === "activo" && <button onClick={() => handleDesactivar(r.id)} style={btnSmallStyle}>Desactivar</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
