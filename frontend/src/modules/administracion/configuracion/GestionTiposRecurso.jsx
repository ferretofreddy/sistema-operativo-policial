      ...
      {loading ? (
        ...
        <div style={tableWrap}>
          <table style={tableStyle}>
            <thead>
              <tr>{["Nombre","Siglas","Descripción","Estado",""].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {tipos.map(t => (
                <tr key={t.id} style={t.estado === "inactivo" ? { opacity: 0.5 } : {}}>
                  ...
                  <td style={tdStyle}>{t.estado === "activo" && <button onClick={() => handleDesactivar(t.id)} style={btnSmallStyle}>Desactivar</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
